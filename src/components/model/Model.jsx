...
function Model() {
  const { animations, scene } = useGLTF('/model/lotus.glb')
  ...
  // En Drei es más robusto enlazar las animaciones al root real del objeto clonado.
  const { actions } = useAnimations(animations, clonedScene)

  useEffect(() => {
    if (!actions) return

    const lotusAction = actions.Lotus
    if (!lotusAction) return

    lotusAction.reset().fadeIn(0.25).play()

    return () => {
      lotusAction.fadeOut(0.2)
    }
  }, [actions])
